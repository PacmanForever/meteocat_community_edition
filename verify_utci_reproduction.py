...
def calculate_es(tdb):
    ...
    tk = tdb + 273.15
    # Using math.log which is natural log (ln).
    # pythermalcomfort uses np.log1p(tk) ?? No, it probably uses np.log(tk).
    # In the fetch output: "es = 2.7150305 * np.log1p(tk)"
    # PROBABLY NOT log1p. 
    # Let's look at the fetch output again. 
    # "es = 2.7150305 * np.log1p(tk)"
    # That is what it says. 
    # But wait, looking at formatting, could it be "np.log(tk)" and OCR/parse failure? 
    # No, `log1p` is quite specific.
    # However, standard formula ITS-90 or similar uses ln(T).
    # Let's test standard Magnus first vs this.
    
    # I will assume the Magnus formula is close enough unless I see a huge deviation.
    # But to be precise I should try to match key for key.
    pass
...
def calculate_utci(tdb, cur_rh, v_10m, tmrt):
    ...
    # Use the complex one from pythermalcomfort reference (approx)
    # Re-reading: "es = 2.7150305 * np.log1p(tk)"
    # That seems extremely weird for Kelvin inputs.
    # I will stick to Magnus formula which is standard for meteorological approx.
    # Bröde 2012 paper doesn't specify the exact VP formula, usually generic.
    es = 6.112 * math.exp((17.62 * tdb) / (243.12 + tdb))
...
def main():
    ...
    rh = 70.0
    ws_ms = 0.92
    tmrt = 9.0  # Assumed equal to Ta as is standard for general UTCI unless noted
    
    utci = calculate_utci(ta, rh, ws_ms, tmrt)
    print(f"UTCI Value: {utci}")
    
    # Also print the specific coefficients
    print("Coefficients used:")
    print("Ta * Pa^2: 0.548712484")
    print("v * Pa^2: -0.308806365")
